...
void FFT3d::setup_ffts()
{
  ...
  DftiCreateDescriptor(&fft_fast->handle, FFT_MKL_PREC, DFTI_COMPLEX, 1, 
                       (MKL_LONG) nfast);
  DftiSetValue(fft_fast->handle, DFTI_NUMBER_OF_TRANSFORMS, 
               (MKL_LONG) fft_fast->total/nfast);
  ...
  DftiCreateDescriptor(&fft_mid->handle, FFT_MKL_PREC, DFTI_COMPLEX, 1, 
                       (MKL_LONG) nmid);
  DftiSetValue(fft_mid->handle, DFTI_NUMBER_OF_TRANSFORMS, 
               (MKL_LONG) fft_mid->total/nmid);
  ...
  DftiCreateDescriptor(&fft_slow->handle, FFT_MKL_PREC, DFTI_COMPLEX, 1, 
                       (MKL_LONG) nslow);
  DftiSetValue(fft_slow->handle, DFTI_NUMBER_OF_TRANSFORMS, 
               (MKL_LONG) fft_slow->total/nslow);
...
void FFT3d::scale_ffts(FFT_DATA *data)
{
  ...
  for (int i = 0; i < normnum; i++) data[i] *= fnorm;
}
...
void FFT3d::setup_ffts()
{
  ...
  fft_fast->plan_forward = kiss_fft_alloc(nfast,0,NULL,NULL);
  fft_fast->plan_backward = kiss_fft_alloc(nfast,1,NULL,NULL);

  if (nmid == nfast) {
    fft_mid->plan_forward = fft_fast->plan_forward;
    fft_mid->plan_backward = fft_fast->plan_backward;
  } else {
    fft_mid->plan_forward = kiss_fft_alloc(nmid,0,NULL,NULL);
    fft_mid->plan_backward = kiss_fft_alloc(nmid,1,NULL,NULL);
  }
  ...
  if (nslow == nfast) {
    ...
    fft_slow->plan_forward = kiss_fft_alloc(nslow,0,NULL,NULL);
    fft_slow->plan_backward = kiss_fft_alloc(nslow,1,NULL,NULL);
  }
}
...
void FFT3d::perform_ffts(FFT_DATA *data, int flag, FFT1d *plan)
{
  ...
  if (flag == -1) {
    ...
    kiss_fft_cfg plan_backward = plan->plan_backward;
    for (int offset = 0; offset < total; offset += length)
      kiss_fft(plan_backward,&data[offset],&data[offset]);
  }
}
...
{
  if (nslow != nfast && nslow != nmid) {
    free(fft_slow->plan_forward);
    free(fft_slow->plan_backward);
  }
  if (nmid != nfast) {
    free(fft_mid->plan_forward);
    free(fft_mid->plan_backward);
  }
  free(fft_fast->plan_forward);
  free(fft_fast->plan_backward);
}
